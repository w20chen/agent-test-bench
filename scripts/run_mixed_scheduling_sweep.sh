#!/usr/bin/env bash
# ──────────────────────────────────────────────────────────────────────────────
# Mixed-Benchmark Scheduling Sweep
#
# Compares two scheduling strategies for a mixed pool of swe-rebench (container)
# and deep-research-bench (host, no_spawn) traces:
#
#   Strategy A ("sequential"): Process N/2 rebench traces first (all cores),
#                              then N/2 deep-research traces (all cores).
#   Strategy B ("interleaved"): Process N/2 rebench + N/2 deep-research
...
#
# Prerequisites:
#   - SOURCE_TRACES_DIR_REBENCH: directory of pre-collected swe-rebench traces
#   - SOURCE_TRACES_DIR_DR: directory of pre-collected deep-research-bench
#     traces (collected with --prompt-template no_spawn)
#   - Docker installed and running (for rebench container traces)
#   - Python 3.12+ with project deps installed
#
# Usage:
#   export SOURCE_TRACES_DIR_REBENCH=/path/to/traces/swe-rebench/model/timestamp
#   export SOURCE_TRACES_DIR_DR=/path/to/traces/deep-research-bench/model/timestamp
...
#     sequential_N<agents>/
#       rebench/   — rebench simulate output
#       dr/        — deep-research simulate output
#     interleaved_N<agents>/
#       rebench/   — rebench simulate output (N/2 agents)
#       dr/        — deep-research simulate output (N/2 agents)
#
...
SOURCE_TRACES_DIR_REBENCH="${SOURCE_TRACES_DIR_REBENCH:-}"
SOURCE_TRACES_DIR_DR="${SOURCE_TRACES_DIR_DR:-}"

if [[ -z "${SOURCE_TRACES_DIR_REBENCH}" ]]; then
  echo "ERROR: SOURCE_TRACES_DIR_REBENCH is not set." >&2
  echo "  export SOURCE_TRACES_DIR_REBENCH=/path/to/swe-rebench/traces" >&2
  exit 1
fi
if [[ -z "${SOURCE_TRACES_DIR_DR}" ]]; then
  echo "ERROR: SOURCE_TRACES_DIR_DR is not set." >&2
  echo "  export SOURCE_TRACES_DIR_DR=/path/to/deep-research-bench/traces" >&2
  exit 1
fi
if [[ ! -d "${SOURCE_TRACES_DIR_REBENCH}" ]]; then
  echo "ERROR: SOURCE_TRACES_DIR_REBENCH does not exist: ${SOURCE_TRACES_DIR_REBENCH}" >&2
  exit 1
fi
if [[ ! -d "${SOURCE_TRACES_DIR_DR}" ]]; then
  echo "ERROR: SOURCE_TRACES_DIR_DR does not exist: ${SOURCE_TRACES_DIR_DR}" >&2
  exit 1
fi
...
# Each round processes N total traces: N/2 rebench + N/2 dr.
# Strategy A: sequential (N/2 rebench → N/2 dr, full cores each phase).
# Strategy B: interleaved (N/2 rebench + N/2 dr in parallel, split cores).
...
# CPU_LIMIT: per-container Docker --cpus quota (container traces only).
...
_run_simulate() {
  ...
  if [[ ${simulate_exit} -ne 0 ]]; then
    echo "  ⚠️  [${label}] simulate exited with code ${simulate_exit} (elapsed: ${run_elapsed_fmt})"
    echo "  Last 20 lines of log:"
    tail -20 "${run_log}" 2>/dev/null | sed 's/^/    /' || true
    return 1
  fi

  echo "  ✓ [${label}] simulate completed (elapsed: ${run_elapsed_fmt})"
...
REBENCH_TRACE_COUNT=$(find "${SOURCE_TRACES_DIR_REBENCH}" -name "trace.jsonl" -type f | wc -l)
DR_TRACE_COUNT=$(find "${SOURCE_TRACES_DIR_DR}" -name "trace.jsonl" -type f | wc -l)
echo "Rebench traces found:     ${REBENCH_TRACE_COUNT}"
echo "DeepResearch traces found: ${DR_TRACE_COUNT}"

if [[ "${REBENCH_TRACE_COUNT}" -eq 0 ]]; then
  echo "ERROR: No trace.jsonl files found under ${SOURCE_TRACES_DIR_REBENCH}" >&2
  exit 1
fi
if [[ "${DR_TRACE_COUNT}" -eq 0 ]]; then
  echo "ERROR: No trace.jsonl files found under ${SOURCE_TRACES_DIR_DR}" >&2
  exit 1
fi
...
HALF_MAX_N=$((MAX_N / 2))
if [[ "${REBENCH_TRACE_COUNT}" -lt "${HALF_MAX_N}" ]]; then
  echo "  ⚠️  Rebench trace count (${REBENCH_TRACE_COUNT}) < HALF_MAX_N (${HALF_MAX_N})."
  echo "     Interleaved mode at N=${MAX_N} needs ${HALF_MAX_N} rebench traces per side."
fi
if [[ "${DR_TRACE_COUNT}" -lt "${HALF_MAX_N}" ]]; then
  echo "  ⚠️  DeepResearch trace count (${DR_TRACE_COUNT}) < HALF_MAX_N (${HALF_MAX_N})."
  echo "     Interleaved mode at N=${MAX_N} needs ${HALF_MAX_N} dr traces per side."
fi
...
if [[ "${TOTAL_CORES}" -gt "${HOST_CORES}" ]]; then
  echo "  ⚠️  TOTAL_CORES (${TOTAL_CORES}) > host cores (${HOST_CORES})."
...
if [[ "${EXISTING_CONTAINERS}" -gt 0 ]]; then
  echo "  ⚠️  ${EXISTING_CONTAINERS} existing container(s) running."
...
{
  echo "Mixed Scheduling Sweep — started at ${STARTED_AT}"
  echo ""
  echo "Configuration:"
  echo "  Rebench traces:       ${SOURCE_TRACES_DIR_REBENCH} (${REBENCH_TRACE_COUNT} traces)"
  echo "  DeepResearch traces:  ${SOURCE_TRACES_DIR_DR} (${DR_TRACE_COUNT} traces)"
  ...
  echo "  Workers:              ${WORKERS}"
  echo "  Task source (rebench): ${TASK_SOURCE_REBENCH}"
  echo "  Task source (dr):      ${TASK_SOURCE_DR}"
  echo "  Sweep values (N):     ${SWEEP_VALUES}"
  echo ""
  echo "Strategies:"
  echo "  A) sequential:  N/2 rebench agents → N/2 dr agents (sequential, full cores)"
  echo "  B) interleaved: N/2 rebench + N/2 dr agents simultaneously (split cores)"
  echo ""
} | tee "${SUMMARY_FILE}"
...
for N in ${SWEEP_VALUES}; do
  ...
  # ═══════════════════════════════════════════════════════════════════════
  # Strategy A: Sequential — all rebench first, then all deep research
  # ═══════════════════════════════════════════════════════════════════════
  _hr "Strategy A (sequential): N=${N} (${HALF_N} rebench → ${HALF_N} dr)"

  SEQ_OUTPUT_DIR="${BASE_OUTPUT_DIR}/sequential_N${N}"
  SEQ_REBENCH_DIR="${SEQ_OUTPUT_DIR}/rebench"
  SEQ_DR_DIR="${SEQ_OUTPUT_DIR}/dr"
  mkdir -p "${SEQ_REBENCH_DIR}" "${SEQ_DR_DIR}"

  SEQ_START=$(date +%s)

  # Phase 1: N/2 rebench (all cores)
  echo "[$(date +%H:%M:%S)] Phase 1/2: Running ${HALF_N} rebench agents (all cores)..."
  REBENCH_OK=0
  if _run_simulate \
    "${SOURCE_TRACES_DIR_REBENCH}" \
    "${SEQ_REBENCH_DIR}" \
    "${HALF_N}" \
    "seq-rebench" \
    "${CORE_RANGE_FULL}" \
  ...
  REBENCH_ELAPSED=$(cat "${SEQ_REBENCH_DIR}/.elapsed_s" 2>/dev/null || echo "N/A")
  DR_ELAPSED=$(cat "${SEQ_DR_DIR}/.elapsed_s" 2>/dev/null || echo "N/A")

  echo "  Strategy A (sequential) N=${N}: done (elapsed: ${SEQ_ELAPSED_FMT})"
  echo "    Rebench phase: ${REBENCH_ELAPSED}s, DR phase: ${DR_ELAPSED}s"

  {
    echo ""
    echo "N=${N}  strategy=sequential  elapsed=${SEQ_ELAPSED_FMT}  elapsed_s=${SEQ_ELAPSED}"
    echo "  rebench_s=${REBENCH_ELAPSED}  dr_s=${DR_ELAPSED}"
    echo "  rebench_ok=${REBENCH_OK}  dr_ok=${DR_OK}"
    echo "  output: ${SEQ_OUTPUT_DIR}"
  } | tee -a "${SUMMARY_FILE}"

  # ═══════════════════════════════════════════════════════════════════════
  # Strategy B: Interleaved — rebench and dr simultaneously
  # ═══════════════════════════════════════════════════════════════════════
  _hr "Strategy B (interleaved): N=${N} (${HALF_N} rebench + ${HALF_N} dr parallel)"

  INTERLEAVED_OUTPUT_DIR="${BASE_OUTPUT_DIR}/interleaved_N${N}"
  INTERLEAVED_REBENCH_DIR="${INTERLEAVED_OUTPUT_DIR}/rebench"
  INTERLEAVED_DR_DIR="${INTERLEAVED_OUTPUT_DIR}/dr"
  mkdir -p "${INTERLEAVED_REBENCH_DIR}" "${INTERLEAVED_DR_DIR}"

  INTERLEAVED_START=$(date +%s)

  # Launch both simultaneously in background.
  # Rebench gets left half of cores, deep research gets right half.

  REBENCH_BG_OK=0
  DR_BG_OK=0

  echo "[$(date +%H:%M:%S)] Launching rebench (${HALF_N} agents, cores ${CORE_RANGE_LEFT})..."
  _run_simulate \
    "${SOURCE_TRACES_DIR_REBENCH}" \
    "${INTERLEAVED_REBENCH_DIR}" \
    "${HALF_N}" \
    "int-rebench" \
    "${CORE_RANGE_LEFT}" \
    "${TASK_SOURCE_REBENCH}" \
  ...
  echo "[$(date +%H:%M:%S)] Launching dr (${HALF_N} agents, cores ${CORE_RANGE_RIGHT})..."
  _run_simulate \
    "${SOURCE_TRACES_DIR_DR}" \
    "${INTERLEAVED_DR_DIR}" \
    "${HALF_N}" \
  ...
  echo "  Rebench pid: ${REBENCH_PID}, DR pid: ${DR_PID}"

  # Wait for both to complete.
  if wait "${REBENCH_PID}"; then
    REBENCH_BG_OK=1
    echo "[$(date +%H:%M:%S)] Rebench (${HALF_N} agents) completed."
  else
    echo "[$(date +%H:%M:%S)] Rebench (${HALF_N} agents) FAILED."
  fi
  # Remove from tracked PIDs now that it's done.
  for i in "${!_BG_PIDS[@]}"; do
    if [[ "${_BG_PIDS[$i]}" == "${REBENCH_PID}" ]]; then
      unset '_BG_PIDS[$i]'
      break
    fi
  done
  ...
  if wait "${DR_PID}"; then
    DR_BG_OK=1
    echo "[$(date +%H:%M:%S)] DeepResearch (${HALF_N} agents) completed."
  else
    echo "[$(date +%H:%M:%S)] DeepResearch (${HALF_N} agents) FAILED."
  fi
  for i in "${!_BG_PIDS[@]}"; do
    if [[ "${_BG_PIDS[$i]}" == "${DR_PID}" ]]; then
      unset '_BG_PIDS[$i]'
      break
    fi
  done
  ...
  REBENCH_BG_ELAPSED=$(cat "${INTERLEAVED_REBENCH_DIR}/.elapsed_s" 2>/dev/null || echo "N/A")
  DR_BG_ELAPSED=$(cat "${INTERLEAVED_DR_DIR}/.elapsed_s" 2>/dev/null || echo "N/A")

  echo "  Strategy B (interleaved) N=${N}: done (elapsed: ${INTERLEAVED_ELAPSED_FMT})"
  echo "    Rebench: ${REBENCH_BG_ELAPSED}s, DR: ${DR_BG_ELAPSED}s"

  {
    echo ""
    echo "N=${N}  strategy=interleaved  elapsed=${INTERLEAVED_ELAPSED_FMT}  elapsed_s=${INTERLEAVED_ELAPSED}"
    echo "  rebench_s=${REBENCH_BG_ELAPSED}  dr_s=${DR_BG_ELAPSED}"
    echo "  rebench_ok=${REBENCH_BG_OK}  dr_ok=${DR_BG_OK}"
