...
def _fill_meta_from_source_trace(meta: dict[str, Any]) -> dict[str, Any]:
    """Extract model and benchmark names from the ``source_trace`` path when
    simulate-mode metadata carries ``model: "unknown"``."""
    source = meta.get("source_trace", "")
    if not source:
        return meta

    # Path pattern: traces/<benchmark>/<model>/<timestamp>/<instance>/attempt_N/trace.jsonl
    parts = source.replace("\\", "/").split("/")
    meta = dict(meta)  # shallow copy

    # Extract benchmark (second path segment)
    if meta.get("benchmark", "?") == "?" and len(parts) >= 2:
        bm_slug = parts[1]  # e.g. "swebench_verified"
        # Normalize common slug patterns
        slug_map = {
        ...
        meta["benchmark"] = slug_map.get(bm_slug, bm_slug)

    # Extract model (third path segment)
    if meta.get("model", "unknown") == "unknown" and len(parts) >= 3:
        meta["model"] = parts[2]

    # Also try source_model
    if meta.get("source_model", "unknown") == "unknown" and len(parts) >= 3:
        meta["source_model"] = parts[2]

    return meta


def _normalize_simulate_events(records: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Normalize simulate-mode traces so they carry the same record types
    and field-level detail as collect-mode traces.

    Collect-mode traces carry fine-grained ``type: "event"`` entries
    (``llm_call_start``, ``llm_call_end``, ``tool_exec_start``,
    ``tool_exec_end``) alongside ``type: "action"`` records with rich
    metadata.  Simulate traces only have ``type: "action"`` with
    ``ts_start``/``ts_end`` and replay-specific fields.  This function
    bridges the gap so downstream code (the Gantt renderer, stats bar,
    and any future event-driven panels) can treat both modes uniformly.

    Specific fixes applied:

    * **Metadata**: fills ``model`` and ``benchmark`` from the source-trace
      path when they are ``"unknown"`` / ``"?"``.
    * **Action ``instance_id``**: fills from ``agent_id`` when empty.
    * **Action data**: adds ``tool_call_id`` (from ``action_id``) for
      tool-exec actions; adds ``llm_call_time_ms`` / ``llm_timing_source``
      for llm-call actions to match the collect schema.
    * **Events**: synthesizes ``llm_call_start``, ``llm_call_end``,
      ``tool_exec_start``, ``tool_exec_end`` events from the action records.
    * **Summary**: adds ``elapsed_s`` if missing.
    """
    # Detect: if we already have event records, this is a collect trace.
    has_events = any(r.get("type") == "event" for r in records)
    if has_events:
        return records

    # Only synthesize if we're in simulate mode (has trace_metadata.mode == "simulate")
    meta = next((r for r in records if r.get("type") == "trace_metadata"), {})
    if meta.get("mode") != "simulate":
        return records

    # ── Fix metadata ──────────────────────────────────────────────
    meta = _fill_meta_from_source_trace(meta)
    ...
    for rec in records:
        ...
        # Write (possibly enriched) metadata at its original position
        if rtype == "trace_metadata":
            synthesized.append(meta)
            meta_written = True
            continue

        if rtype != "action":
            # Fix summary fields
            if rtype == "summary" and "elapsed_s" not in rec:
                rec = dict(rec)
                rec["elapsed_s"] = rec.get("elapsed_s", 0.0)
            synthesized.append(rec)
            continue

        # ── Fix action fields ──────────────────────────────────
        rec = dict(rec)  # shallow copy so we can mutate
        data = dict(rec.get("data") or {})
        rec["data"] = data

        # instance_id: fill from agent_id if empty
        if not rec.get("instance_id"):
            rec["instance_id"] = rec.get("agent_id", "")

        atype = rec.get("action_type", "")
        ts_s = rec.get("ts_start", 0)
        ts_e = rec.get("ts_end", 0)
        if ts_s <= 0 and ts_e <= 0:
            synthesized.append(rec)
            continue
        ...
        # ── Synthesize events ──────────────────────────────────
        if atype == "llm_call":
            # Add collect-format timing fields to action data
            llm_latency_ms = data.get("llm_latency_ms", 0)
            if "llm_call_time_ms" not in data:
                data["llm_call_time_ms"] = llm_latency_ms
            if "llm_wall_latency_ms" not in data:
                data["llm_wall_latency_ms"] = llm_latency_ms
            if "llm_timing_source" not in data:
                data["llm_timing_source"] = "wall_clock_ms"

            # llm_call_start event
            synthesized.append({
                "type": "event",
                "agent_id": agent_id,
                ...
                "category": "LLM",
                "data": {"messages_in": data.get("messages_in", [])},
                "ts": ts_s,
                "iteration": iteration,
            })
            # llm_call_end event
            synthesized.append({
                ...
                "data": {
                    ...
                    "llm_call_time_ms": llm_latency_ms,
                    "llm_wall_latency_ms": llm_latency_ms,
                    "llm_timing_source": "wall_clock_ms",
                    "finish_reason": "tool_calls",
                    "is_malformed_retry": False,
            ...
            # Add tool_call_id if missing (collect traces have it)
            if "tool_call_id" not in data:
                data["tool_call_id"] = action_id

            tool_name = data.get("tool_name", "")
            tool_args_raw = data.get("tool_args", "")
            if isinstance(tool_args_raw, dict):
                tool_args_raw = json.dumps(tool_args_raw, ensure_ascii=False)
            args_preview = str(tool_args_raw)[:200] if tool_args_raw else ""

            # tool_exec_start event
            synthesized.append({
                "type": "event",
                "agent_id": agent_id,
                "program_id": program_id,
            ...
            result_preview = str(tool_result_raw)[:200] if tool_result_raw else ""
            # tool_exec_end event
            synthesized.append({
                "type": "event",
                "agent_id": agent_id,
        ...
        # Always keep the (possibly enriched) action record
        synthesized.append(rec)
...
def generate_html(attempt_dir: Path) -> str:
    ...
    records = _normalize_simulate_events(records)
